...
import { 
  ...
  MessageSquare, Paperclip, Send, Target, Link as LinkIcon, Activity
} from "lucide-react";
...
export default function SalesTaskDetailPage() {
  ...
  const [comments, setComments] = useState<any[]>([
    { id: 1, author: "Sales Manager", text: "Please ensure you have the latest deck before the meeting.", time: "Yesterday, 2:15 PM", isInternal: true }
  ]);
  ...
  if (!task) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center h-[calc(100vh-4rem)] bg-slate-50">
        ...
        <Button variant="outline" onClick={() => router.push('/sales/tasks')}>Return to Tasks</Button>
      </div>
    );
  }
  ...
  const navigateToRelated = () => {
    if (!task.relatedEntity?.id) return;
    
    if (task.relatedEntity.type === 'Client' || task.relatedEntity.type === 'Lead') {
      router.push(`/sales/client-connect`);
    } else if (task.relatedEntity.type === 'Property') {
    ...
    <div className="flex-1 overflow-y-auto bg-slate-50 p-6 h-[calc(100vh-4rem)]">
      <div className="max-w-5xl mx-auto space-y-6 pb-20 animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4 mb-2">
          <Button variant="ghost" size="icon" onClick={() => router.push('/sales/tasks')} className="rounded-full shrink-0 bg-white shadow-sm border border-slate-200 hover:bg-slate-100">
                ...
                <div className="divide-y divide-slate-100">
                  ...
                  <div className="p-4 flex justify-between items-center bg-white">
                    ...
                    <span className="text-xs font-bold text-slate-900">
                      {task.history && task.history[0] ? new Date(task.history[0].date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric'}) : new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric'})}
                    </span>
                  </div>
                  <div className="p-4 flex justify-between items-center bg-white">
            ...
            {task.relatedEntity && (
              <Card className="border-slate-200 shadow-sm overflow-hidden">
                <CardHeader className="p-5 border-b border-slate-100 bg-slate-50">
                  <CardTitle className="text-sm font-bold uppercase tracking-widest text-slate-500 flex items-center gap-2">
                    <LinkIcon className="h-4 w-4" /> Related {task.relatedEntity.type}
                  </CardTitle>
                </CardHeader>
                <CardContent className="p-5 bg-white space-y-4">
                  <div>
                    <h4 className="font-bold text-slate-900 mb-1">{task.relatedEntity.name}</h4>
                    <p className="text-xs font-medium text-slate-500">Entity Type: {task.relatedEntity.type}</p>
                  </div>
                  {task.relatedEntity.id && (
                    <Button 
                      variant="outline" 
                      className="w-full text-xs font-bold text-indigo-600 border-indigo-200 hover:bg-indigo-50"
                      onClick={navigateToRelated}
                    >
                      View {task.relatedEntity.type}
                    </Button>
                  )}
                </CardContent>
              </Card>
                  ...
                  <div className="relative flex items-start gap-4">
                    ...
                    <div className="pt-0.5">
                      ...
                      <span className="text-[9px] font-bold text-slate-400 mt-1 block">
                        {task.history && task.history[0] ? new Date(task.history[0].date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric'}) : '17 Jun'}
                      </span>
                    </div>
                  </div>
