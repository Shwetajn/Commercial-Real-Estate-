    ...
    return (
      <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in zoom-in-95 duration-500 py-12">
        ...
        <div className="text-center pt-8">
          <Button variant="ghost" onClick={() => router.push('/admin/inventory')}>Cancel & Return</Button>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
          ...
          <div className="flex gap-3">
            {currentStep < totalSteps ? (
              ...
              <Button onClick={handleAdminSubmit} disabled={isSubmitting}>
                {isSubmitting ? "Submitting..." : "Submit Property (Approved)"} <CheckCircle2 className="h-4 w-4 ml-2" />
              </Button>
            )}
          </div>
        </div>
      ...
      <Dialog open={showSuccessModal} onOpenChange={setShowSuccessModal}>
        <DialogContent className="sm:max-w-md rounded-3xl p-8 border-slate-200/60 shadow-2xl flex flex-col items-center text-center">
          ...
          <h2 className="text-2xl font-bold text-slate-900 mb-2">Property Added & Approved</h2>
          <p className="text-slate-500 font-medium">Your {flowType === 'commercial' ? 'commercial' : 'co-working'} property has been successfully added directly to the active inventory.</p>
        </DialogContent>
      </Dialog>
    </div>
  );
